...
type
  ...
  public
    EdObject: TATSynEdit;
    EdProps: array of TATEditorPaintingItemProp;
    EdCharSize: TPoint;
    EdMainText: boolean;
    EdScrollHorz,
    EdScrollVert: TATEditorScrollInfo;
    procedure Execute; override;
  end;

var
  PaintThreads: array[0..cPaintThreads-1] of TPaintThread;

{ TPaintThread }

procedure TPaintThread.Execute;
var
  Prop: PATEditorPaintingItemProp;
  iProp: integer;
begin
  for iProp:= 0 to High(EdProps) do
  begin
    Prop:= @EdProps[iProp];
    EdObject.DoPaintLine(
      Prop^.Bitmap.Canvas,
      Prop^.BitmapRect,
      EdCharSize,
      EdMainText,
...
function IsPaintThreadsBusy: boolean;
var
  busy: boolean;
  i: integer;
begin
  for i:= 0 to cPaintThreads-1 do
    if not PaintThreads[i].Finished then
      exit(true);
  Result:= false;
end;
...
var
  ...
  RectLine: TRect;
  GapItem: TATGapItem;
  DividedParts: TATIntPairArray;
  NWrapIndex, NWrapIndexDummy,
  NLineCount, NPropCount,
  iProp, iThread: integer;
begin
  ...
  repeat
    ...
    PropPtr^.Bitmap.SetSize(
      PropPtr^.BitmapRect.Width,
      PropPtr^.BitmapRect.Height
      );
  ...
  //update LineBottom
  if NPropCount=0 then exit;
  NWrapIndex:= Props[NPropCount-1].WrapIndex;
  FLineBottom:= FWrapInfo[NWrapIndex].NLineIndex;

  //threads
  DividedParts:= DivideRangeByParts(0, NPropCount-1, cPaintThreads);
  for iThread:= 0 to High(DividedParts) do
    with PaintThreads[iThread] do
    begin
      EdObject:= Self;
      SetLength(EdProps, DividedParts[iThread].NTo-DividedParts[iThread].NFrom+1);
      for iProp:= 0 to High(EdProps) do
        EdProps[iProp]:= Props[DividedParts[iThread].NFrom+iProp];
      EdCharSize:= ACharSize;
      EdMainText:= AMainText;
      EdScrollHorz:= AScrollHorz;
      EdScrollVert:= AScrollVert;
      Start;
    end;

  repeat
    Sleep(5);
  until Application.Terminated or not IsPaintThreadsBusy;
  ...
  begin
    ...
    C.Draw(
      PropPtr^.LineRect.Left,
      PropPtr^.LineRect.Top,
      PropPtr^.Bitmap
      );
...
initialization
  ...
  for iLoop:= 0 to High(PaintThreads) do
    PaintThreads[iLoop]:= TPaintThread.Create(true);

finalization

  for iLoop:= 0 to High(PaintThreads) do
  begin
    if not PaintThreads[iLoop].Finished then
      PaintThreads[iLoop].WaitFor;
    FreeAndNil(PaintThreads[iLoop]);
  end;
